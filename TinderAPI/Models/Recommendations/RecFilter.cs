...
namespace TinderAPI.Models.Recommendations
{
    ...
    public sealed class StringRecFilter : BaseRecFilter
    {
        ...
        public override bool Match(string bio)
        {
            ...
            /*if (COMPARE.IndexOf(bio, _filter, CompareOptions.IgnoreCase) != -1)
            {
                LogManager.Enqueue(
                    API.LOG_NAME,
                    EntryType.DEBUG,
                    "Recommendation auto-swiped",
                    Name
                );
                return true;
            }
    ...
    public sealed class ArrayRecFilter : BaseRecFilter
    {
        ...
        public override bool Match(string bio)
        {
            ...
            /*if (_filter.Any(f => COMPARE.IndexOf(bio, f, CompareOptions.IgnoreCase) != -1))
            {
                LogManager.Enqueue(
                    API.LOG_NAME,
                    EntryType.DEBUG,
